...
void printFractals(appConfig &config){
    ...
    while (true){
        ...
        if (config.rows != screen.rows() || config.columns != screen.columns()){
        ...
        for (int i = 0; i < config.matrixSize ; ++i){
            ...
            while (!eventQueue.empty()){
                optional<Term::Event> eventOpt = eventQueue.front();
                eventQueue.pop();
                if (eventOpt.has_value()){
                    Term::Event event = eventOpt.value();
                    ...
                    if (key == '-' ) config.zoom /= 1.1;
                    if (key == Term::Key::Esc) {config.zoom = 1; config.xOffset = -1; config.yOffset = 0;} 
                    if (key == 'x') {config.state = 0; return;}
                    }
                }
            }
...
void printMainMenu(appConfig &config){
    ...
    menuObject resolution("Image resolution", 'l', vector<string> {"auto","16","32","64","128","255"});
            ...
            if (eventOpt.has_value()){
                ...
                if (event.type() == Term::Event::Type::Key) {
                    ...
                    if (key == Term::Key::Enter && menuList[selectedItem].label == "Start") {
                        config.state = 1; 
                        config.matrixSize = typeid(stoi(menuList[1].getValue())).name() == "int" ? stoi(menuList[1].getValue()) : (screen.rows() < screen.columns() ? screen.rows() : screen.columns()) ;
